    ...
    if path.endswith(".gz"):
        with open(path, "rb") as compressed_file:
            with tqdm(
                ...
                with gzip.GzipFile(
                    fileobj=(CallbackIOWrapper(t.update, compressed_file, "read"))
                ) as gz:
                    for x in gz:
        ...
        with open(path, "r") as f:
            with tqdm(total=size, unit="B", unit_scale=True, unit_divisor=1024) as t:
                for x in CallbackIOWrapper(t.update, f, "read"):
...
def construct_rib_tables(
    ...
    if progress:
        print("Constructing peer index table", end="")
    ...
    if progress:
        print("Constructing rib tables")

    for text_entry in tqdm(entries, disable=not progress):
...
def write_binary_mrt(
    ...
    with tqdm(unit="b", unit_scale=True, unit_divisor=1025, disable=not progress) as t:
        with open(output_path, "wb") as outfile:
            total = mrt_bytes_write(
                CallbackIOWrapper(t.update, outfile, "write"),
                timestamp,
                peer_index_table,
                rib_tables,
            )
